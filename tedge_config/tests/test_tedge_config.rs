...
"#;
    ...
    assert_eq!(
        config.query(MqttExternalCAfileSetting)?,
...
"#;
    ...
    let updated_mqtt_external_cafile = "ca.pem";
...

    {
        ...
        config.update(MqttExternalBindAddressSetting, updated_mqtt_external_bind_address.to_string())?;
        config.update(MqttExternalBindInterfaceSetting, updated_mqtt_external_bind_interface.to_string())?;
        config.update(MqttExternalCAfileSetting, FilePath::from(updated_mqtt_external_cafile))?;
        config.update(MqttExternalCertfileSetting, FilePath::from(updated_mqtt_external_certfile))?;
        config.update(MqttExternalKeyfileSetting, FilePath::from(updated_mqtt_external_keyfile))?;
...

    {
        ...
        assert_eq!(config.query(MqttExternalPortSetting)?, updated_mqtt_external_port);
        assert_eq!(
            config.query(MqttExternalBindAddressSetting)?.as_str(), 
        ...
        assert_eq!(
            config.query(MqttExternalCAfileSetting)?, 
            FilePath::from(updated_mqtt_external_cafile)
        );
        assert_eq!(
            config.query(MqttExternalCertfileSetting)?, 
